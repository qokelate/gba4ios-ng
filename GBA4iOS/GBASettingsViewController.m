...
- (void)viewDidLoad
{
    ...
    s2.on = [[WebServer sharedInstance] isRunning];
    [s2 addTarget:self action:@selector(webserver:) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *s1 = [[UIBarButtonItem alloc] initWithCustomView:s2];
    self.navigationItem.leftBarButtonItems = @[s1];
}

- (void)webserver: (UISwitch *)sender
{
