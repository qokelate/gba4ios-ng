...
#import "../TweakBox/TweakBoxView.h"
...
@property (unsafe_unretained, nonatomic) UILabel *fpslb;
@end
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(willEnterForeground:) name:UIApplicationWillEnterForegroundNotification object:nil];
    ...
    lb.frame = CGRectMake(12, 12, 50, 24);
    [self.view addSubview:(_fpslb = lb)];
   ...
   CFTimeInterval timePassed = link.timestamp - _timestamp;
    ...
    if(timePassed >= 1.f) {
       ...
       CGFloat fps = _scheduleTimes/timePassed;
        ...
        char tmp1[64];
        size_t t1 = sprintf(tmp1, "%.1f", fps);
        _fpslb.text = [[NSString alloc] initWithBytesNoCopy:tmp1 length:t1 encoding:NSUTF8StringEncoding freeWhenDone:NO];
...
- (void)controllerInputDidPressMenuButton:(id)controllerInput
{
    ...
    [self.pausedActionSheet addButtonWithTitle:@"TweakBox"];
    ...
    void (^selectionHandler)(UIActionSheet *actionSheet, NSInteger buttonIndex) = ^(UIActionSheet *actionSheet, NSInteger buttonIndex) {
        ...
        if ([title isEqualToString:@"TweakBox"])
        {
            ...
            TweakBoxView *v1 = [[TweakBoxView alloc] initWithFrame:self.view.bounds];
