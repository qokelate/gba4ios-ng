...
- (void)viewDidLoad
{
    ...
    if (self.cheat)
    {
        ...
        self.codeTextView.text = [self formatedCodes];
...
- (NSArray *)codesFromTextView
{
    ...
    NSMutableString *text = [[[self.codeTextView.text stringByRemovingWhitespace] uppercaseString] mutableCopy];
...
- (NSMutableString *)formatedCodes
{
    NSMutableString *m1 = [NSMutableString stringWithCapacity:128];
    [[self codesFromTextView] enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [m1 appendString:obj];
        [m1 appendString:@"\n"];
    }];
...
{
    textView.text = [self formatedCodes];
}
